...
class IMG_Stegno:
    ...
    def decode_frame2(self, d_F2):
        d_F3 = Frame(root)
        myfiles = tkinter.filedialog.askopenfilename(filetypes=([('png', '*.png'), ('All Files', '*.*')]))
        if not myfiles:
            messagebox.showerror("Error", "You have selected nothing! ")
        else:
    ...
    def enc_fun(self, text_a, myImg):
        ...
        if (len(data) == 0):
            ...
            messagebox.showinfo("Success", "Encoding Successful\nYour encoded file has been saved.")
